...
const buildProposalSummary = (proposal) => {
  ...
  const lines = [
    ...
    parsed.fareType && parsed.fareAmount ? `Fare: ${parsed.fareType === 'COLLECT' ? 'Collect' : 'Min'} $${Number(parsed.fareAmount)}` : null,
    parsed.paymentMethod && parsed.paymentMethod !== 'UNKNOWN' ? `Payment: ${parsed.paymentMethod.replace(/_/g, '/')}` : null,
    parsed.boa ? 'BOA: Yes' : null,
    Array.isArray(parsed.missing) && parsed.missing.length ? `Missing: ${parsed.missing.join(', ')}` : null,
