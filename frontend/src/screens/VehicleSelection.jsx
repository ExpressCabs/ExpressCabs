...
const VehicleSelection = ({
  ...
  return (
    <>
      <h2 className="mb-4 text-center text-2xl font-semibold text-gray-800">Select Your Vehicle</h2>

      <div className="mb-4 rounded-xl border border-slate-200 bg-white p-4 text-sm text-gray-700 shadow-sm">
        <div className="flex flex-col gap-3 text-left sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-1 flex flex-wrap gap-x-4 gap-y-1 text-sm text-gray-600">
              ...
              <span>Time: {durationMin ? Math.round(durationMin) : '--'} min</span>
            </div>
          </div>
          <div className="grid gap-1 text-left text-sm text-gray-600 sm:min-w-[220px]">
            <div>Rates: ${tariff.perKm}/km or ${tariff.perMin}/min</div>
            <div>Booking fee: ${BOOKING_FEE.toFixed(2)}</div>
            {airportPickup && <div>Airport fee: ${MEL_AIRPORT_PICKUP_FEE.toFixed(2)}</div>}
            {hasTolls && <div>Toll charges applied</div>}
            <div>Traffic factor applied</div>
          </div>
        </div>
      </div>
